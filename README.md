...
- **[plane_slam_demo](examples/plane_slam_demo.rs)** -- plane SLAM with a USER-DEFINED component: `UnitVec`, a 2-DOF unit direction on the sphere, built inside the example from public pieces -- `#[arael(component)]`, the `Component` lifecycle trait, a cached chart (`compute =`), a `symbolic =` embed with `let` intermediates, and a declared Jacobian cache (`deriv =`). SE3 poses and plane landmarks (unit normal + distance) with odometry and plane observations; the same model [benchmarks/plane](benchmarks/plane) races against g2o, Ceres, GTSAM, SymForce and factrs.
