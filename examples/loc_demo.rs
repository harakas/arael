// Localization demo -- same as SLAM but with known (fixed) landmarks.
//
// Landmarks are not optimized (pos: vect3f, not Param<vect3f>). Since the
// known landmark positions provide an absolute reference frame, there is no
// gauge freedom and absolute pose errors are meaningful. No GPS needed.
//
// The frine constraint uses a remote block (pose.hb_pose) -- the hessian
