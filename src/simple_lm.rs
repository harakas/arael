...
pub struct LmConfig<T> {
    /// Convergence threshold: stop when cost decrease is below this for 3 consecutive steps.
    pub abs_precision: T,
    /// Maximum number of LM iterations (including inner damping retries).
    pub max_iters: usize,
    /// Initial damping parameter lambda.
    pub initial_lambda: T,
    /// Print per-iteration cost and timing to stderr.
    pub verbose: bool,
}
...
impl<T: Float> Default for LmConfig<T> {
    fn default() -> Self {
        LmConfig {
            abs_precision: T::from(0.01).unwrap(),
            max_iters: 100,
            initial_lambda: T::from(0.001).unwrap(),
            verbose: false,
        }
    }
}
    ...
    while iter < config.max_iters && !done {
        ...
        while inner < INNER_LOOPS && iter < config.max_iters {
            ...
            if new_cost.is_finite() && new_cost < end_cost {
                ...
                if iter >= 5 && (end_cost - new_cost) < config.abs_precision {
                    small_count += 1;
                    if small_count >= 3 {
                        done = true;
                    }
...
mod tests {
    ...
    fn solve_rosenbrock() {
        ...
        let result = solve(
            &[-1.0, 1.0],
            &mut p,
            &LmConfig { abs_precision: 1e-12, max_iters: 500, initial_lambda: 0.001, verbose: false },
        );
    ...
    fn solve_f32_quadratic() {
        ...
        let result = solve_f32(
            &[0.0f32, 0.0],
            &mut p,
            &LmConfig { abs_precision: 1e-4, max_iters: 100, initial_lambda: 0.001, verbose: false },
        );
    ...
    fn dense_vs_sparse_hessian_equal() {
        ...
        let config = LmConfig { abs_precision: 1e-10, max_iters: 100, initial_lambda: 0.001, verbose: false };
