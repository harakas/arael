...
| `#[arael(root)]` | mark the top-level Model. Generates the `LmProblem` and `RootModel` impls (unlocking LmProblem's `solve_with` / `solve_dense` / `solve_sparse`), manages indices, owns the update cycle |
