...
class GraphPosesVec:
    """View of `poses` (vec of Pose2); wrappers stay valid per the
...
class GraphEdgesVec:
    """View of `edges` (vec of Edge); wrappers stay valid per the
    C++ contract, mutating while iterating is undefined."""
...
class Graph:
    ...
    def solve_sparse(self, cfg=None):
        ...
        r = LmResult()
        return self._solved(
            _f.graph_solve_sparse(self._p, ctypes.byref(cfg),
                                      ctypes.byref(r)), r)
    ...
    def last_report(self):
        ...
        return _f.graph_last_report(self._p, False).decode()

    def last_pretty_report(self):
        """last_report() with colour and box-drawing glyphs."""
        return _f.graph_last_report(self._p, True).decode()
