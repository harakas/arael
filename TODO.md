...
- **arael**: unify the LmConfig / LambdaDriver configuration surface -- two competing systems configure the same algorithm now. The damping parameters `initial_lambda` and `lambda_floor` live in LmConfig, but the LambdaDriver *owns* all lambda decisions and reads those two fields out of LmConfig in its `start()` (DefaultLambdaDriver and NielsenLambdaDriver both do). So "what is the initial lambda / floor" is answered in LmConfig while "how does lambda evolve" is answered in the driver -- split-brain, and a driver that ignored config.initial_lambda would silently make the LmConfig field a lie. Options: (a) move initial_lambda + lambda_floor onto the driver constructors (LmConfig becomes purely termination + verbose; plain solve entry points build a DefaultLambdaDriver from defaults) -- clean ownership but the 90% "just set initial_lambda" case now needs a driver; (b) make the driver a field of LmConfig (`driver: Box<dyn LambdaDriver>` or an enum) so LmConfig stays the single front door -- but that breaks LmConfig's Copy/simple-literal ergonomics and adds boxing; (c) keep the split but make the coupling explicit in docs and have the driver's `start()` be the sole authority (LmConfig's lambda fields become "defaults the built-in drivers read", documented as such). Decide before a learned/third driver ships, since each new driver otherwise re-invents the config-vs-self boundary. Ties into F4/R11 (LmStatus, final_lambda, exposing lambda_up/down/ceiling -- those knobs already moved into DefaultLambdaDriver, so F4's "expose them" half is a driver-API question now, not an LmConfig one).
