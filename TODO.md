...
- **arael**: LambdaDriver follow-ups. The driver abstraction and NielsenLambdaDriver are DONE (LambdaDriver trait, LambdaStep/LambdaState context, *_driven entry points; on BAL Ladybug-49 Nielsen eliminates all 12 rejections and cuts the total 821 -> 539 ms). Remaining:
  - Consider making NielsenLambdaDriver the default after validating it across the pgo and sketch workloads (it must not regress the pose graphs' 5-7-step solves or the sketch's tiny systems).
  - Ladybug-138 still stops ~1.4% above Ceres's optimum under ANY schedule/floor combination -- investigate Ceres's Jacobi column scaling of the trust region as the remaining difference (arael's multiplicative diag damping is Marquardt scaling, similar but not identical), or whether the two land in genuinely different basins.
