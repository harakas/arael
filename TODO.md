...
- **arael**: adaptive LM damping schedule (Nielsen gain-ratio style, or -- per user preference -- a domain-trained NN lambda controller; the verbose trace already yields (lambda, predicted vs actual reduction, accept/reject) training tuples for free). Hard evidence from the BAL benchmark (benchmarks/bal): on Ladybug-49 arael's fixed multiply/divide schedule burns 12 rejected attempts while Ceres's gain-ratio trust region takes 22 steps with zero rejections; on Ladybug-138 NO fixed (lambda0, floor) pair works well -- the default floor spirals into gauge-driven Cholesky failures and gives up 2.8% high, while the 1e-4 floor that fixes it forces 54 crawling steps (Ceres: 22) and makes Ladybug-49's f32 hit the iteration cap. The schedule is the single biggest lever on every measured BA total.

- **benchmarks/bal**: Ladybug-1723 needs a tighter shared termination criterion before it can be a fair race: with the common 1e-5 tolerances neither arael nor Ceres converges -- they stop at plateaus 1.9% apart (arael's the lower cost) and the mutual validation gate cannot pass. Also: arael f32 loses positive definiteness immediately at this scale (485k params).
