...
- **arael-faer**: the Schur setup got slower with the one-column kernel work, and it scales with the problem. Setup is 1st-iter minus full-iter; on the slam benchmark it went 0.68 -> 0.86 ms at 60 poses, 1.96 -> 2.32 at 120, 9.39 -> 18.80 at 300, and 59.71 -> 66.88 on the Pi Zero W (the Pi 5 at 60 poses was flat, 3.82 -> 3.86). At 300 poses that cancels the per-iteration win in total ms (140.95 -> 141.00) even though full-iter dropped 44.16 -> 40.76. The external systems' 1st-iter was flat or down in the same runs, so it is arael's, not the machine. Cause unverified. What `schur_symbolic` gained is five per-observer arrays and a separate pass turning `pair_dst` block indices into `pair_off` value offsets; only the second is O(pairs), which is how the measurement scales. If that is it, the conversion folds into the loop that already writes those entries -- `val_ptr` for a target block is final by the time that column's pair targets are scattered -- and the extra pass goes away.
