...
- **arael**: port the COO-free first iteration to the other scalar-CSC backends. SparseFaer's first compute now builds its pattern and position map from the model's block structure (`hessian_pattern_requires_compute` gate: `collect_hessian_cells` -> `csc_from_cells` -> `accumulate_hessian_positions`); SparseEigen, SparseCholmod, and SparseCholmodSupernodal (cholmod-gpl) still discover their pattern with a COO pass on every first compute. The machinery is backend-agnostic (lives in simple_lm; produces an ordinary CscMatrix), so each backend's compute() needs the same first-call branch SparseFaer got. Measured stake on the slam bench: faer's first iteration dropped 5.8 -> 4.8 ms at 60 poses and 83.9 -> 78.5 at 300, while cholmod-gpl's stayed flat (5.9 / ~105) -- most of its remaining first-iter gap to faer is this. One check per backend: the tile-expanded pattern carries ~1.2% explicit zeros (no steady-state cost measured on faer's supernodal LLT; verify CHOLMOD and Eigen tolerate them as ordinary nonzeros before adopting).

- **arael-faer**: S-factorization backend for the Schur route. faer's sparse LLT numeric on S (n = 1800, 69% dense upper at slam-300) takes 23.1 ms/iter -- the largest piece of the 41.1 ms Schur-route linear solve -- while g2o's CHOLMOD supernodal factors its equivalent reduced system in ~10-12 ms (G2O_STATS). Deferred by decision 2026-07-11. Things to try: `CholeskySymbolicParams` supernodal/simplicial regime hints (force supernodal or tune the threshold -- maybe faer just picks the wrong regime for a 69%-dense S), ordering on S (AMD vs natural vs pose-chain), a dense-LLT comparison point, and band+border (the kept graph is a pose chain + loop closures; see SCHUR.md band notes).
