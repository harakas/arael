...
- **DOF computation ownership in Sketch**: Move async DOF lifecycle into Sketch itself. Mutations kick off async computation internally. `sketch.dof()` blocks and waits if result not ready. GUI reads `sketch.cached_dof` for non-blocking display. Eliminates the external async plumbing (dof_input/dof_output/dof_display, bincode serialize/deserialize copy, poll_dof). -- DONE (resolved by deleting the async instead: the fast rank made DOF 34-54ms on the largest sketches, so Sketch/SketchCell own a generation-keyed rank cache (`ensure_rank`/`cached_rank`) and every consumer computes synchronously through it; the worker thread, channels, bincode copy, poll_dof and the GUI-side bg_rank/dof_display are gone)
