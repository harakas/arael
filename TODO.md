...
- **arael-faer**: settle the GEMM kernel question on x86. nano-gemm now takes the shapes with no unrolled kernel (`--example gemmbench`), but the hot shapes -- (6,3,6) SLAM, (9,3,9) BAL -- still go to our const-generic kernels, because on aarch64/NEON nano-gemm loses to them by 1.2-1.4x: it reaches its microkernel through a function pointer, and at these sizes that indirect call costs more than the arithmetic it dispatches. Unmeasured: whether AVX-512 flips that. nano-gemm's published numbers come from its AVX-512 microkernels, which are behind its `x86-v4` feature (we forward it, x86 only), and `gemmbench` prints the SIMD path it took so the two runs can be told apart. If nano wins the hot shapes there, the routing should change on that hardware and the feature becomes load-bearing rather than benchmark-only -- note it is NOT currently forwarded from the `arael` root crate, so downstream cannot reach it. Also still unmeasured: the `gemm` crate (the general dense kernel) for the same job.
