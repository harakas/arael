...
class LmResult(_f.LmResultRaw):
    ...
    def steps(self):
        ...
        if not self._detail:
            return []
        n = _f.line_result_steps(self._detail, None, 0)
        if not n:
            return []
        buf = (LmStep * n)()
        _f.line_result_steps(self._detail, buf, n)
        return list(buf)
...
def _cov_query(c, fn, p, cap):
    ...
    if n < 0:
        raise AraelError(
            n, (_f.line_cov_error(c) or b"").decode())
    return buf
...
class Covariance:
    ...
    def plan(self):
        ...
        p = CovPlan()
        _f.line_cov_plan(self._h, ctypes.byref(p))
        return p

    def free(self):
        if self._h:
            _f.line_cov_free(self._h)
            self._h = None
...
class Line:
    ...
    def _solved(self, code, res):
        if code < 0:
            ...
            if code == -1:
                fl = SolveFailure()
                if _f.line_last_failure(self._p, ctypes.byref(fl)):
                    failure = fl
    ...
    def assemble_covariance(self, mode=CovMode.ALL_MARGINALS,
        ...
        c = ctypes.c_void_p()
        code = _f.line_assemble_covariance_with(
