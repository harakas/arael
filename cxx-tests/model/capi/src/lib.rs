...
/// fit_last_error.
#[no_mangle]
pub unsafe extern "C" fn fit_last_failure(h: *const FitHandle, out: *mut CSolveFailure) -> bool {
    let hh = &*h;
    *out = hh.failure;
    hh.failure.kind != 0
}
...
/// The per-step records are read with fit_result_steps.
#[repr(C)]
...
#[no_mangle]
pub unsafe extern "C" fn fit_result_steps(d: *const ResultDetail, out: *mut CLmStep, cap: u64) -> u64 {
...
#[no_mangle]
pub extern "C" fn fit_set_log_level(level: u32) {
...
/// struct name); the table sums to fit_cost. Sorts the
/// table by label, stores it on the handle, and returns the row count
/// (-2: panic, text via fit_last_error). Read the rows with
/// fit_cost_table_name / _value.
#[no_mangle]
pub unsafe extern "C" fn fit_cost_table(h: *mut FitHandle) -> i32 {
    let hh = &mut *h;
    match catch_unwind(AssertUnwindSafe(|| {
        let mut params = Vec::new();
        hh.model.serialize(&mut params);
...
/// fit_cost_table call.
#[no_mangle]
pub unsafe extern "C" fn fit_cost_table_name(h: *const FitHandle, i: u32) -> *const c_char {
    let hh = &*h;
...
#[no_mangle]
pub unsafe extern "C" fn fit_cost_table_value(h: *const FitHandle, i: u32) -> f64 {
    let hh = &*h;
    hh.cost_table[i as usize].1
}

/// A computed Jacobian, owned by the caller (release with
/// fit_jac_free). A snapshot of the current parameters: later
...
fn jac_text(j: &mut FitJac, msg: &str) {
    j.text = CString::new(msg.replace('\0', " ")).unwrap_or_default();
}

/// Error text of the last failed query on this Jacobian.
#[no_mangle]
pub unsafe extern "C" fn fit_jac_error(j: *const FitJac) -> *const c_char {
    (&*j).text.as_ptr()
}

/// Release a Jacobian. Null is fine.
#[no_mangle]
pub unsafe extern "C" fn fit_jac_free(j: *mut FitJac) {
    if !j.is_null() {
        drop(Box::from_raw(j));
    }
}
...
/// box in `out`; -2 (panic, text via fit_last_error).
#[no_mangle]
pub unsafe extern "C" fn fit_calc_jacobian(h: *mut FitHandle, out: *mut *mut FitJac) -> i32 {
    let hh = &mut *h;
    *out = std::ptr::null_mut();
    match catch_unwind(AssertUnwindSafe(|| {
        ...
        Ok(j) => {
            *out = Box::into_raw(Box::new(FitJac {
                jac: j,
                text: CString::default(),
            }));
            set_text(hh, "");
...
#[no_mangle]
pub unsafe extern "C" fn fit_jac_num_residuals(j: *const FitJac) -> u64 {
    (&*j).jac.num_residuals() as u64
}

/// Number of parameter columns.
#[no_mangle]
pub unsafe extern "C" fn fit_jac_num_params(j: *const FitJac) -> u64 {
...
/// sizes the buffer); -2 (panic, text via fit_jac_error).
#[no_mangle]
...
/// sizes the buffer); -2 (panic, text via fit_jac_error).
#[no_mangle]
pub unsafe extern "C" fn fit_jac_column_l2_norms(j: *mut FitJac, out: *mut f64, cap: u64) -> i64 {
    let jj = &mut *j;
...
/// fit_cov_free). Independent of later assemblies; entity
...
#[no_mangle]
pub unsafe extern "C" fn fit_cov_free(c: *mut FitCov) {
    if !c.is_null() {
        drop(Box::from_raw(c));
    }
}
