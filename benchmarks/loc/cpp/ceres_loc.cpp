// Ceres runner for the localization benchmark. Reads the scene exported by
// the Rust harness (identical problem), builds the four factor types as
// AutoDiffCostFunctions, and solves. Pose blocks are plain 6-vectors
// [x, y, z, roll, pitch, yaw] with the rotation built from the euler angles
// inside each residual -- matching arael's SimpleEulerAngleParam (no
// manifold) exactly. Landmarks are FIXED constants, so the bearing factor is
// single-variable over the pose. No GPS. Every residual equals
// scene::reference_cost (asserted by the harness via the initial cost).
//
...
struct BearingCost {
    const double *lm, *mf2r, *camera_pos, *isigma; double scale;
    template <typename T> bool operator()(const T* pose, T* r) const {
        Eigen::Matrix<T, 3, 3> mr2w = euler_to_rot(pose + 3);
        Eigen::Matrix<T, 3, 1> d(T(lm[0]) - pose[0], T(lm[1]) - pose[1], T(lm[2]) - pose[2]);
...
static void build_problem(Scene& s, std::vector<double>& poses, ceres::Problem& problem) {
    ...
    for (int i = 0; i < s.n_frines; i++) {
        problem.AddResidualBlock(new ceres::AutoDiffCostFunction<BearingCost, 2, 6>(
            new BearingCost{&s.lm_pos[3 * s.f_lm[i]], &s.f_mf2r[9 * i], &s.f_camera_pos[3 * i],
                            &s.f_isigma[2 * i], s.g.frine_isigma_scale}),
            nullptr, &poses[6 * s.f_pose[i]]);
    }
    for (int i = 0; i < s.n_odo; i++) {
