...
//   relative camera-center RMSE under 1e-3 (bundle adjustment has a
...
// Run the Ceres subprocess: JSON protocol line on stdout (initial_cost
// cross-checked against the reference), 9-value camera lines followed
// by 3-value point lines in params_out.
fn run_ceres(ds_path: &str, linsolver: &str, n_cams: usize, n_points: usize,
    ...
    let initial = get("initial_cost").expect("initial_cost");
    assert!(((initial - expected_initial) / expected_initial).abs() < 1e-9,
        "ceres initial cost {} disagrees with reference {}", initial, expected_initial);
    ...
    for (name, rel_path, dense_schur_ok, floor) in &datasets {
        ...
        let converged = |c: &Cell| {
            (c.cost - best) / best < 1e-2
                && bal::aligned_relative_rmse(&bal::camera_centers(&c.cameras), &best_centers) < 1e-3
        };
        ...
        println!("validation: {}/{} systems at the common optimum ({:.4}: cost within 1%, \
                  similarity-aligned relative camera-center RMSE < 1e-3), anchored by {} external system(s)",
