...
void run(const char* g2o, const char* poses_out) {
    ...
    printf("{\"solve_ms\": %.3f, \"first_iter_ms\": %.3f, \"second_run_ms\": %.3f, \"second_accepted\": %d, \"first_attempts\": %d, \"first_accepted\": %d, \"iterations\": %d, \"accepted\": %d, "
           "\"initial_cost\": %.6f, \"cpus_allowed\": \"%s\"}\n",
           full.ms, first.ms, two.ms, two.accepted, first.total, first.accepted, full.total, full.accepted, initial_cost(poses, edges), cpus.c_str());
}
